...
export function either<L, R>(l?: L, r?: R) {
  ...
  if (exists(l) && !exists(r)) {
    return Either.left<L, R>(l as L);
  }
  //  if (!exists(l) && exists(r)) {
  return Either.right<L, R>(r as R); // fix unreachable codepath error
  //  }
...
*/
  ...
  caseOf<T>(pattern: EitherPatterns<L, R, T>) {
    return this.type === EitherType.Right ?
      pattern.right(this.r as R) :
      pattern.left(this.l as L);
  }
