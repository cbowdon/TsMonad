...
describe("Maybe", () => {
    ...
    it("Constructors", () => {
        ...
        assert.ok(
            Maybe.maybe<string>(null).caseOf({
                just: (s) => false,
                nothing: () => true,
            })
        );
    ...
    it("valueOrCompute", () => {
        assert.strictEqual(
            Maybe.just(10).valueOrCompute(() => 20),
            10
        );

        assert.strictEqual(
            Maybe.nothing<number>().valueOrCompute(() => 20),
            20
        );

        assert.strictEqual(
            Maybe.just(10).valueOrCompute<any>(() => {
                throw new Error();
            }),
            10
        );
        ...
        assert.throws(() =>
            Maybe.nothing().valueOrCompute<any>(() => {
                throw new Error();
            })
        );
    });
    ...
    it("valueOrThrow", () => {
        ...
        assert.throws(
            () => Maybe.nothing<number>().valueOrThrow(err),
            (e: Error) => e === err
        );
    });
