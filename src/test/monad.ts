...
import { Writer, writer } from "../writer.js";
...
describe("Type class laws", () => {
    ...
    it("Functor 1: fmap id = id", () => {
        _.each([Maybe.just(20), Maybe.nothing<number>()], (t) => assert.ok(t.equals(t.fmap((x) => x))));

        _.each([Either.right<string, number>(20), Either.left<string, number>("oook")], (t) =>
            assert.ok(t.equals(t.fmap((x) => x)))
        );

        _.each([Writer.writer(["(^_^)"], 99)], (t) => assert.ok(t.equals(t.fmap((x) => x))));
    });
    ...
    it("Functor 2: fmap (f . g) = fmap f . fmap g", () => {
        ...
        _.each([Either.right<string, number>(10), Either.left<string, number>("oook")], (t) => {
            var lhs = t.fmap(f).fmap(g),
                rhs = t.fmap((x) => g(f(x)));
            assert.ok(lhs.equals(rhs));
        });
    ...
    it("Monad 1: left identity", () => {
        ...
        assert.ok(Writer.writer<number, number>([], n).bind(fw).equals(fw(n)));
    });
