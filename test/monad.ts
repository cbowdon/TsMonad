...
describe('Type class laws', () => {
  ...
  it('Eq', () => {
    ...
    assert.ok(either<string, TestEq>(undefined, wizzard()).equals(either<string, TestEq>(undefined, wizzard())));
    assert.ok(!either<string, TestEq>(undefined, wizzard()).equals(either<string, TestEq>(undefined, wizard())));

    assert.ok(maybe([1, 2, 3, 4]).equals(maybe([1, 2, 3, 4])));
    assert.ok(!maybe([1, 2, 3, 4]).equals(maybe([1, 2, 3, 4, 5])));
    assert.ok(either<string, number[]>(undefined, [1, 2, 3, 4]).equals(either<string, number[]>(undefined, [1, 2, 3, 4])));
    assert.ok(!either<string, number[]>(undefined, [1, 2, 3, 4]).equals(either<string, number[]>(undefined, [1, 2, 3, 99])));
